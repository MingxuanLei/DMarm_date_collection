...
try:
    import teleoperation_shijiao_new as core
...
class TeleoperationWorker(QObject):
    ...
    def start_or_switch(
        ...
        if core is None:
            self.log_signal.emit(f"[ERR] 导入 teleoperation_shijiao_new.py 失败: {CORE_IMPORT_ERROR}")
            return
    ...
    def _apply_runtime_options(
        ...
        # 这些是 teleoperation_shijiao_new.py 中的全局开关。
    ...
    def _run(
        ...
        try:
            with contextlib.redirect_stdout(writer), contextlib.redirect_stderr(writer):
                ...
                print("=" * 70)
                print("七电机弱双向遥操作示教 GUI 已启动（直接调用 teleoperation_shijiao_new.py）")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("七电机双向力反馈示教记录回放界面（含速度记录）")
        ...
        if core is None:
            self.append_log(f"[ERR] 导入 teleoperation_shijiao_new.py 失败: {CORE_IMPORT_ERROR}")
            self.append_log("[ERR] 请确认 GUI 文件与 teleoperation_shijiao_new.py 位于同一目录。")
        else:
            self.append_log("[INFO] GUI 初始化完成。当前界面直接调用 teleoperation_shijiao_new.py，第7工具电机弱反馈默认开启。")
    ...
    def _build_ui(self) -> None:
        ...
        self.record_file_edit.setPlaceholderText("留空：自动保存为 trajectory/teach_record_时间.csv")
        self.replay_file_edit = QLineEdit()
        self.replay_file_edit.setPlaceholderText("留空：自动读取本次记录文件或 trajectory 中最新记录")
    ...
    def start_or_switch(self, cmd: str) -> None:
        if core is None:
            QMessageBox.critical(self, "错误", f"无法导入 teleoperation_shijiao_new.py：\n{CORE_IMPORT_ERROR}")
            return

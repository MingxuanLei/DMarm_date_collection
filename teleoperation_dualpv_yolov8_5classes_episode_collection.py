...
def parse_args():
    parser = argparse.ArgumentParser(description="七电机弱双向力反馈同构遥操作、RGB-D+YOLOv8记录与示教回放程序")
    parser.add_argument(
        "--mode",
...
def create_d435i_recorder(enable_camera_recording=True):
    """创建并预启动 D435i RGB-D + YOLOv8 录制器。失败时返回 None，不影响遥操作。"""
    ...
    if not recorder.start_camera():
        print("[D435i][WARN] D435i RGB-D相机启动失败，本次只记录机械臂 CSV，不录制视频")
        try:
            recorder.close()
        except Exception:
            pass

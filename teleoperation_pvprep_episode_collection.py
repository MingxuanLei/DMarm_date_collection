...
def make_pv_move_command(target, target_dh_q, pv_vel, tool_target_q=None):
    """构造准备模式下使用的PV预定位命令。

    target: "master" / "slave" / "both"
    target_dh_q: 6个DH关节角，单位rad
    pv_vel: PV速度限制
    tool_target_q: 第7工具电机目标角；None表示第7电机保持当前位置
    """
    q = np.asarray(target_dh_q, dtype=float).reshape(6)
    return {
        "type": PV_CMD_TYPE,
        "target": str(target),
        "target_dh_q": [float(x) for x in q],
        "pv_vel": float(pv_vel),
        "tool_target_q": None if tool_target_q is None else float(tool_target_q),
    }
...
def handle_prepare_pv_command(master, slave, cmd):
    ...
    target_dh_q = cmd.get("target_dh_q", None)
    pv_vel = float(cmd.get("pv_vel", PV_VEL_LIM))
    tool_target_q = cmd.get("tool_target_q", None)

    if target_dh_q is None:
        print("[PV][ERR] 未提供目标DH关节角")
        return False
    ...
    if target == PV_TARGET_BOTH:
        ...
        def _move_master():
            results["master"] = move_controller_to_fixed_pv(master, "主端", target_dh_q, pv_vel, tool_target_q)

        def _move_slave():
            results["slave"] = move_controller_to_fixed_pv(slave, "从端", target_dh_q, pv_vel, tool_target_q)

        print("[PV] 主从同时预定位：主端和从端将并行切换到PV固定目标")

...
try:
    import teleoperation_pvprep_episode_collection as core
...
class TeleoperationWorker(QObject):
    ...
    def start_or_switch(
        ...
        if core is None:
            self.log_signal.emit(f"[ERR] 导入 teleoperation_pvprep_episode_collection.py 失败: {CORE_IMPORT_ERROR}")
            return
    ...
    def send_pv_move(self, target: str, target_dh_q, pv_vel: float, tool_target_q=None) -> None:
        if core is None:
            self.log_signal.emit(f"[ERR] 导入 teleoperation_pvprep_episode_collection.py 失败: {CORE_IMPORT_ERROR}")
            return
        if not self.running:
            ...
            return
        try:
            cmd = core.make_pv_move_command(target, target_dh_q, pv_vel, tool_target_q)
            self._cmd_queue.put(cmd)
            self.log_signal.emit(f"[PV][CMD] 已发送PV预定位命令: {target}")
    ...
    def _apply_runtime_options(
        ...
        # 这些是 teleoperation_pvprep_episode_collection.py 中的全局开关。
    ...
    def _run(
        ...
        try:
            with contextlib.redirect_stdout(writer), contextlib.redirect_stderr(writer):
                ...
                print("=" * 70)
                print("七电机弱双向遥操作示教 GUI 已启动（直接调用 teleoperation_pvprep_episode_collection.py）")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("七电机双向力反馈示教记录回放界面（紧凑PV预定位+Episode+D435i RGB）")
        ...
        if core is None:
            self.append_log(f"[ERR] 导入 teleoperation_pvprep_episode_collection.py 失败: {CORE_IMPORT_ERROR}")
            self.append_log("[ERR] 请确认 GUI 文件与 teleoperation_pvprep_episode_collection.py 位于同一目录。")
        else:
            self.append_log("[INFO] GUI 初始化完成。当前界面直接调用 teleoperation_pvprep_episode_collection.py，支持采集前PV固定姿态预定位。")
    ...
    def _build_pv_prepare_group(self, root_layout: QVBoxLayout) -> None:
        """采集前PV固定姿态预定位区域：紧凑表格版。"""
        pv_group = QGroupBox("采集前PV固定姿态预定位（紧凑表格）")
        layout = QGridLayout(pv_group)
        layout.setHorizontalSpacing(8)
        layout.setVerticalSpacing(4)
        layout.setContentsMargins(8, 8, 8, 8)

        default_q = [3.10, -1.60, -3.10, 0.00, 1.56, 0.25]
        self.pv_q_spin: List[QDoubleSpinBox] = []
        self.pv_q_deg_label: List[QLabel] = []

        # 目标角紧凑布局：每个关节占 3 列：名称 + rad输入 + deg显示。
        # q1~q3 放第一行，q4~q6 放第二行，减少纵向高度。
        for i, value in enumerate(default_q):
            spin = QDoubleSpinBox()
            ...
            deg_label.setMinimumWidth(58)
            deg_label.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)

            self.pv_q_spin.append(spin)
            self.pv_q_deg_label.append(deg_label)
        ...
        # 第7工具电机和PV速度限制也放到同一行，避免继续向下堆叠。
        self.check_pv_tool_target = QCheckBox("控制第7工具电机")
        self.check_pv_tool_target.setChecked(True)
        self.pv_tool_spin = QDoubleSpinBox()
        self.pv_tool_spin.setRange(-2.0 * math.pi, 2.0 * math.pi)
        self.pv_tool_spin.setDecimals(4)
        self.pv_tool_spin.setSingleStep(0.01)
        self.pv_tool_spin.setValue(-2.9)
        self.pv_tool_spin.setFixedWidth(92)
        self.pv_tool_spin.valueChanged.connect(self.update_pv_deg_labels)
        self.pv_tool_deg_label = QLabel("0.00°")
        self.pv_tool_deg_label.setMinimumWidth(58)

        self.pv_vel_spin = QDoubleSpinBox()
        self.pv_vel_spin.setRange(0.01, 5.0)
        self.pv_vel_spin.setDecimals(3)
        self.pv_vel_spin.setSingleStep(0.05)
        self.pv_vel_spin.setValue(0.3)
        self.pv_vel_spin.setFixedWidth(92)

        layout.addWidget(self.check_pv_tool_target, 2, 0, 1, 2)
        layout.addWidget(self.pv_tool_spin, 2, 2)
        layout.addWidget(self.pv_tool_deg_label, 2, 3)
        layout.addWidget(QLabel("PV速度限制"), 2, 4)
        layout.addWidget(self.pv_vel_spin, 2, 5)

        hint = QLabel("流程：先点“4 采集前准备模式”，再执行PV预定位；到位后手动切换到遥操作/记录。")
        hint.setWordWrap(True)
        layout.addWidget(hint, 3, 0, 1, 9)

        self.btn_pv_master = QPushButton("主端移动到目标")
        self.btn_pv_slave = QPushButton("从端移动到目标")
        self.btn_pv_both = QPushButton("主从同时移动到目标")
        ...
        layout.addWidget(self.btn_pv_master, 4, 0, 1, 3)
        layout.addWidget(self.btn_pv_slave, 4, 3, 1, 3)
        layout.addWidget(self.btn_pv_both, 4, 6, 1, 3)
    ...
    def update_pv_deg_labels(self) -> None:
        if hasattr(self, "pv_q_spin"):
            for spin, label in zip(self.pv_q_spin, self.pv_q_deg_label):
                label.setText(f"{spin.value() * 180.0 / math.pi:.2f}")
        if hasattr(self, "pv_tool_spin") and hasattr(self, "pv_tool_deg_label"):
            self.pv_tool_deg_label.setText(f"{self.pv_tool_spin.value() * 180.0 / math.pi:.2f}")

    def get_pv_target_dh_q(self) -> List[float]:
        return [float(spin.value()) for spin in self.pv_q_spin]

    def send_pv_prepare_command(self, target: str) -> None:
        if core is None:
            QMessageBox.critical(self, "错误", f"无法导入 teleoperation_pvprep_episode_collection.py：\n{CORE_IMPORT_ERROR}")
            return
        if not self.worker.running:
            QMessageBox.warning(self, "提示", "请先点击“4 采集前准备模式”，等待主从机械臂初始化完成后再执行PV预定位。")
            return
        target_dh_q = self.get_pv_target_dh_q()
        pv_vel = float(self.pv_vel_spin.value())
        tool_target_q = float(self.pv_tool_spin.value()) if self.check_pv_tool_target.isChecked() else None
        self.worker.send_pv_move(target, target_dh_q, pv_vel, tool_target_q)

...
from intelrealsense_recorder import D435iRecorder
...
try:
    import teleoperation_data_collection as core
...
class TeleoperationWorker(QObject):
    ...
    def start_or_switch(
        ...
        if core is None:
            self.log_signal.emit(f"[ERR] 导入 teleoperation_data_collection.py 失败: {CORE_IMPORT_ERROR}")
            return
    ...
    def _apply_runtime_options(
        ...
        # 这些是 teleoperation_data_collection.py 中的全局开关。
    ...
    def _run(
        ...
        try:
            with contextlib.redirect_stdout(writer), contextlib.redirect_stderr(writer):
                ...
                print("=" * 70)
                print("七电机弱双向遥操作示教 GUI 已启动（直接调用 teleoperation_data_collection.py）")
                ...
                print(f"D435i RGB视频目录：{core.VIDEO_DIR}")
                print("=" * 70)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("七电机双向力反馈示教记录回放界面（含D435i RGB数据采集）")
        ...
        if core is None:
            self.append_log(f"[ERR] 导入 teleoperation_data_collection.py 失败: {CORE_IMPORT_ERROR}")
            self.append_log("[ERR] 请确认 GUI 文件与 teleoperation_data_collection.py 位于同一目录。")
        else:
            self.append_log("[INFO] GUI 初始化完成。当前界面直接调用 teleoperation_data_collection.py，第7工具电机弱反馈默认开启。")
            self.append_log(f"[INFO] 数据根目录: {core.DATA_DIR}")
            self.append_log(f"[INFO] 轨迹默认保存目录: {core.TRAJECTORY_DIR}")
            self.append_log(f"[INFO] D435i RGB视频默认保存目录: {core.VIDEO_DIR}")
    ...
    def _build_ui(self) -> None:
        ...
        camera_layout.addWidget(QLabel("保存：data/video/teach_record_时间_color.mp4、camera_timestamps.csv、camera_meta.json"), 0, 1)
    ...
    def start_or_switch(self, cmd: str) -> None:
        if core is None:
            QMessageBox.critical(self, "错误", f"无法导入 teleoperation_data_collection.py：\n{CORE_IMPORT_ERROR}")
            return

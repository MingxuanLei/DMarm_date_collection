...
from intelrealsense_yolov8_5classes_episode_recorder import D435iRecorder
...
try:
    import teleoperation_dualpv_yolov8_5classes_episode_collection as core
...
class TeleoperationWorker(QObject):
    ...
    def start_or_switch(
        ...
        if core is None:
            self.log_signal.emit(f"[ERR] 导入 teleoperation_dualpv_yolov8_episode_collection.py 失败: {CORE_IMPORT_ERROR}")
            return
    ...
    def _apply_runtime_options(
        ...
        # 这些是 teleoperation_dualpv_yolov8_episode_collection.py 中的全局开关。
    ...
    def _run(
        ...
        try:
            with contextlib.redirect_stdout(writer), contextlib.redirect_stderr(writer):
                ...
                print("=" * 70)
                print("七电机弱双向遥操作示教 GUI 已启动（直接调用 teleoperation_dualpv_yolov8_episode_collection.py）")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("七电机双向力反馈示教记录回放界面（主从独立PV预定位+Episode+D435i RGB-D+YOLOv8）")
        ...
        if core is None:
            self.append_log(f"[ERR] 导入 teleoperation_dualpv_yolov8_episode_collection.py 失败: {CORE_IMPORT_ERROR}")
            self.append_log("[ERR] 请确认 GUI 文件与 teleoperation_dualpv_yolov8_episode_collection.py 位于同一目录。")
        else:
            self.append_log("[INFO] GUI 初始化完成。当前界面直接调用 teleoperation_dualpv_yolov8_episode_collection.py，支持采集前主从独立PV固定姿态预定位，并支持D435i RGB-D + YOLOv8实时检测测距。")
            self.append_log(f"[INFO] 数据根目录: {core.DATA_DIR}")
            self.append_log("[INFO] 每次记录自动创建 data/episode_xx/trajectory 与 data/episode_xx/video，并保存目标检测CSV。")
    ...
    def _build_ui(self) -> None:
        ...
        # 右列：D435i RGB-D + YOLOv8 实时检测、测距与同步采集
        camera_group = QGroupBox("D435i RGB-D + YOLOv8实时检测、测距与同步采集")
        camera_layout = QGridLayout(camera_group)

        self.check_d435i_recording = QCheckBox("记录模式下同步录制RGB视频、时间戳和检测结果")
        self.check_d435i_recording.setChecked(True)
        camera_layout.addWidget(self.check_d435i_recording, 0, 0, 1, 2)
        camera_layout.addWidget(
            QLabel("保存：color.mp4、camera_timestamps.csv、camera_meta.json、object_detections.csv"),
            0, 2, 1, 3,
        )

        self.check_yolo_detection = QCheckBox("启用YOLOv8实时检测")
        self.check_yolo_detection.setChecked(True)
        self.check_show_xyz = QCheckBox("显示XYZ与空间距离")
        self.check_show_xyz.setChecked(True)

        self.yolo_model_edit = QLineEdit("yolov8n.pt")
        self.yolo_model_edit.setPlaceholderText("例如 yolov8n.pt 或自定义 best.pt")
        self.btn_browse_yolo_model = QPushButton("选择模型")
        self.btn_apply_yolo_model = QPushButton("加载模型")

        self.yolo_conf_spin = QDoubleSpinBox()
        self.yolo_conf_spin.setRange(0.05, 0.99)
        self.yolo_conf_spin.setDecimals(2)
        self.yolo_conf_spin.setSingleStep(0.05)
        self.yolo_conf_spin.setValue(0.50)

        camera_layout.addWidget(self.check_yolo_detection, 1, 0)
        camera_layout.addWidget(self.check_show_xyz, 1, 1)
        camera_layout.addWidget(QLabel("模型"), 1, 2)
        camera_layout.addWidget(self.yolo_model_edit, 1, 3)
        camera_layout.addWidget(self.btn_browse_yolo_model, 1, 4)
        camera_layout.addWidget(self.btn_apply_yolo_model, 1, 5)
        camera_layout.addWidget(QLabel("置信度"), 2, 0)
        camera_layout.addWidget(self.yolo_conf_spin, 2, 1)

        self.yolo_status_label = QLabel("YOLO状态：等待相机初始化")
        self.yolo_detection_label = QLabel("检测结果：暂无")
        self.yolo_detection_label.setWordWrap(True)
        camera_layout.addWidget(self.yolo_status_label, 2, 2, 1, 2)
        camera_layout.addWidget(self.yolo_detection_label, 2, 4, 1, 2)

        self.camera_rgb_label = QLabel("D435i RGB-D画面初始化中...")
        ...
        camera_layout.addWidget(self.camera_rgb_label, 3, 0, 1, 6)
        right_layout.addWidget(camera_group, 0)

        self.btn_browse_yolo_model.clicked.connect(self.browse_yolo_model)
        self.btn_apply_yolo_model.clicked.connect(self.apply_yolo_model)
        self.check_yolo_detection.toggled.connect(self.on_yolo_enabled_changed)
        self.check_show_xyz.toggled.connect(self.on_show_xyz_changed)
        self.yolo_conf_spin.valueChanged.connect(self.on_yolo_conf_changed)
    ...
    def _init_camera_preview(self) -> None:
        """启动 D435i RGB-D 预览和 YOLOv8 后台检测线程。"""
        try:
            ...
            self.camera_recorder = D435iRecorder(
                enable=True,
                color_width=width,
                color_height=height,
                depth_width=width,
                depth_height=height,
                fps=fps,
                enable_yolo=self.check_yolo_detection.isChecked(),
                yolo_model_path=self.yolo_model_edit.text().strip() or "yolov8n.pt",
                yolo_conf=float(self.yolo_conf_spin.value()),
                show_xyz=self.check_show_xyz.isChecked(),
            )
            ok = self.camera_recorder.start_camera()
            if ok:
                self.append_log(f"[D435i] RGB-D实时预览已启动：{width}x{height}@{fps}")
                self.append_log("[YOLO] 后台检测线程已启动；模型加载失败不会影响RGB-D预览和遥操作。")
            else:
                self.append_log("[D435i][WARN] RGB-D实时预览启动失败，请检查相机连接和权限。")
        except Exception as exc:
            self.camera_recorder = None
            self.append_log(f"[D435i][WARN] RGB-D实时预览初始化异常：{exc}")

    def _refresh_camera_preview(self) -> None:
        """在 Qt 主线程中刷新标注画面与检测摘要；YOLO推理不在GUI线程执行。"""
        if self.camera_recorder is None:
            if hasattr(self, "camera_rgb_label"):
                self.camera_rgb_label.setText("D435i RGB-D相机未启动")
            return

        frame_bgr = self.camera_recorder.get_latest_preview()
        if frame_bgr is None:
            self.camera_rgb_label.setText("等待D435i RGB-D画面...")
            return
        ...
        try:
            ...
            status = self.camera_recorder.get_yolo_status()
            infer_ms = status.get("inference_ms")
            infer_text = "" if infer_ms is None else f"，推理 {infer_ms:.1f} ms"
            self.yolo_status_label.setText(
                f"YOLO状态：{status.get('status', '未知')}，目标 {status.get('detection_count', 0)} 个{infer_text}"
            )

            detections = self.camera_recorder.get_latest_detections()
            if detections:
                parts = []
                for det in detections[:5]:
                    ...
                    rng = det.get("range_m")
                    if z is None:
                        parts.append(f"{name} {conf:.2f}: Z=N/A")
                    elif rng is None:
                        parts.append(f"{name} {conf:.2f}: Z={float(z):.3f}m")
                    else:
                        parts.append(f"{name} {conf:.2f}: Z={float(z):.3f}m, R={float(rng):.3f}m")
                self.yolo_detection_label.setText("检测结果：" + " | ".join(parts))
            else:
                self.yolo_detection_label.setText("检测结果：暂无")

        except Exception as exc:
            self.camera_rgb_label.setText(f"D435i RGB-D/YOLO画面显示异常：{exc}")
    ...
    def apply_yolo_model(self) -> None:
        if self.camera_recorder is None:
            self.append_log("[YOLO][WARN] 相机尚未初始化")
            return
        model_path = self.yolo_model_edit.text().strip()
        if not model_path:
            self.append_log("[YOLO][WARN] 模型路径不能为空")
            return
        try:
            self.camera_recorder.set_yolo_model(model_path)
            self.append_log(f"[YOLO] 已请求后台加载模型：{model_path}")
        except Exception as exc:
            self.append_log(f"[YOLO][ERR] 设置模型失败：{exc}")

    def on_yolo_enabled_changed(self, enabled: bool) -> None:
        if self.camera_recorder is not None:
            self.camera_recorder.set_yolo_enabled(enabled)

    def on_show_xyz_changed(self, enabled: bool) -> None:
        if self.camera_recorder is not None:
            self.camera_recorder.set_show_xyz(enabled)

    def on_yolo_conf_changed(self, value: float) -> None:
        if self.camera_recorder is not None:
            self.camera_recorder.set_yolo_confidence(float(value))
    ...
    def send_pv_prepare_command(self, target: str) -> None:
        if core is None:
            QMessageBox.critical(self, "错误", f"无法导入 teleoperation_dualpv_yolov8_episode_collection.py：\n{CORE_IMPORT_ERROR}")
            return
    ...
    def closeEvent(self, event) -> None:
        ...
        try:
            ...
            self.append_log(f"[D435i][WARN] 关闭RGB-D/YOLO相机异常：{exc}")
        event.accept()

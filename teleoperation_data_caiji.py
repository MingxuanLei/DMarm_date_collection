...
try:
    from realsense_recorder import D435iRecorder
...
# CSV 轨迹文件保存到 data/trajectory；D435i 视频保存到 data/video。
# 记录文件默认命名为 teach_record_实时日期时间.csv。
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(SCRIPT_DIR, "data")
TRAJECTORY_DIR = os.path.join(DATA_DIR, "trajectory")
VIDEO_DIR = os.path.join(DATA_DIR, "video")

# D435i 同步录制开关。相机不可用时只打印警告，不影响遥操作。
...
D435I_FPS = 30
D435I_SAVE_COLOR_VIDEO = True
D435I_SAVE_DEPTH_VIS_VIDEO = True
D435I_ALIGN_DEPTH_TO_COLOR = True

# 保存本次程序运行过程中最近一次记录的文件名，便于记录后直接切换到回放。
LAST_RECORD_FILE = None

def ensure_data_dirs():
    """确保 data/trajectory 和 data/video 文件夹存在。"""
    os.makedirs(TRAJECTORY_DIR, exist_ok=True)
    os.makedirs(VIDEO_DIR, exist_ok=True)
    return DATA_DIR

def ensure_trajectory_dir():
    """确保 data/trajectory 文件夹存在，并返回其绝对路径。"""
    ensure_data_dirs()
    return TRAJECTORY_DIR

def ensure_video_dir():
    """确保 data/video 文件夹存在，并返回其绝对路径。"""
    ensure_data_dirs()
    return VIDEO_DIR

def make_teach_record_filename():
    """生成 data/trajectory 文件夹下带实时本地时间戳的轨迹文件路径。"""
...
def resolve_record_file_path(record_file=None):
    """
    将记录文件统一保存到 data/trajectory 文件夹。
...
def resolve_replay_file_path(replay_file=None):
    ...
    2. 如果只给了文件名，则优先从 data/trajectory 文件夹查找；
    3. 如果都不存在，返回 None。
    """
...
def find_latest_teach_record_file(folder=None):
    """自动寻找 data/trajectory 文件夹下最新的 teach_record_*.csv。"""
...
def parse_args():
    ...
    parser.add_argument("--record-file", default=None, help="记录模式保存的 CSV 文件名，默认保存到 data/trajectory 文件夹")
    parser.add_argument("--replay-file", default=None, help="回放模式读取的 CSV 文件路径；若只写文件名，则优先从 data/trajectory 文件夹查找")
    ...
    parser.add_argument("--no-camera-record", action="store_true", help="记录模式下不启动 D435i 同步视频录制")
...
def create_d435i_recorder(enable_camera_recording=True):
    """创建并预启动 D435i 录制器。失败时返回 None，不影响遥操作。"""
    if not enable_camera_recording:
        print("[D435i] 本次未启用 D435i 同步录制")
        return None

    if D435iRecorder is None:
        print(f"[D435i][WARN] 导入 realsense_recorder.D435iRecorder 失败: {REALSENSE_IMPORT_ERROR}")
        print("[D435i][WARN] 将继续运行遥操作，但不会录制视频")
        return None
    ...
    recorder = D435iRecorder(
        ...
        fps=D435I_FPS,
        save_color_video=D435I_SAVE_COLOR_VIDEO,
        save_depth_vis_video=D435I_SAVE_DEPTH_VIS_VIDEO,
        align_depth_to_color=D435I_ALIGN_DEPTH_TO_COLOR,
    )

    if not recorder.start_pipeline():
        print("[D435i][WARN] D435i 启动失败，本次只记录机械臂 CSV，不录制视频")
        try:
            recorder.close()
        except Exception:
            pass
...
def enter_record_mode(record_file, camera_recorder=None):
    ...
    if camera_recorder is not None:
        try:
            video_dir = ensure_video_dir()
            record_stem = make_record_stem(record_file)
            ok = camera_recorder.start_record(video_dir, record_stem, record_start_time)
            if not ok:
...
def leave_record_mode(record_handle, record_file, camera_recorder=None):
    if camera_recorder is not None:
        try:
...
def run_teleop_record_session(cmd_queue, initial_mode, record_file, enable_camera_recording=None):
    """
    运行遥操作/遥操作记录会话。
    ...
    try:
        ...
        if current_mode == RUN_MODE_RECORD:
            record_handle, writer, record_start_time = enter_record_mode(record_file, camera_recorder=camera_recorder)
        else:
            print()
        ...
        while True:
            ...
            if cmd == CMD_EXIT:
                ...
                if current_mode != RUN_MODE_TELEOP:
                    record_handle, writer, record_start_time = leave_record_mode(record_handle, record_file, camera_recorder=camera_recorder)
    ...
    try:
        while current_cmd != CMD_EXIT:
            ...
            if current_mode in (RUN_MODE_TELEOP, RUN_MODE_RECORD):
                ...
                current_cmd = run_teleop_record_session(cmd_queue, current_mode, record_file, enable_camera_recording=(not args.no_camera_record))
                ...
                if replay_file is None:
                    print("[ERR] 未指定回放文件，也没有在 data/trajectory 文件夹中找到 teach_record_*.csv。")

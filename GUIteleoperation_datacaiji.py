...
try:
    import teleoperation_data_caiji as core
...
class TeleoperationWorker(QObject):
    ...
    def start_or_switch(
        ...
        if core is None:
            self.log_signal.emit(f"[ERR] 导入 teleoperation_data_caiji.py 失败: {CORE_IMPORT_ERROR}")
            return
    ...
    def _apply_runtime_options(
        ...
        # 这些是 teleoperation_data_caiji.py 中的全局开关。
    ...
    def _run(
        ...
        try:
            with contextlib.redirect_stdout(writer), contextlib.redirect_stderr(writer):
                ...
                print("=" * 70)
                print("七电机弱双向遥操作示教 GUI 已启动（直接调用 teleoperation_data_caiji.py）")
                ...
                print(f"D435i同步录制：{'开启' if core.ENABLE_D435I_RECORDING else '关闭'}")
                print(f"CSV轨迹目录：{core.TRAJECTORY_DIR}")
                print(f"D435i视频目录：{core.VIDEO_DIR}")
                print("=" * 70)
                ...
                while current_cmd != core.CMD_EXIT and not self._stop_event.is_set():
                    ...
                    if current_mode in (core.RUN_MODE_TELEOP, core.RUN_MODE_RECORD):
                        resolved_record_file = core.resolve_record_file_path(record_file)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("七电机双向力反馈示教记录回放界面（含D435i数据采集）")
        ...
        if core is None:
            self.append_log(f"[ERR] 导入 teleoperation_data_caiji.py 失败: {CORE_IMPORT_ERROR}")
            self.append_log("[ERR] 请确认 GUI 文件与 teleoperation_data_caiji.py 位于同一目录。")
        else:
            self.append_log("[INFO] GUI 初始化完成。当前界面直接调用 teleoperation_data_caiji.py，第7工具电机弱反馈默认开启。")
            self.append_log(f"[INFO] 数据根目录: {core.DATA_DIR}")
            self.append_log(f"[INFO] 轨迹默认保存目录: {core.TRAJECTORY_DIR}")
            self.append_log(f"[INFO] D435i视频默认保存目录: {core.VIDEO_DIR}")
    ...
    def _build_ui(self) -> None:
        ...
        self.record_dir_label = QLabel("轨迹目录：")
        self.video_dir_label = QLabel("视频目录：")
        if core is not None:
            self.record_dir_label.setText(f"轨迹目录：{core.TRAJECTORY_DIR}")
            self.video_dir_label.setText(f"视频目录：{core.VIDEO_DIR}")
        ...
        self.record_file_edit.setPlaceholderText("留空：自动保存为 data/trajectory/teach_record_时间.csv")
        self.replay_file_edit = QLineEdit()
        self.replay_file_edit.setPlaceholderText("留空：自动读取本次记录文件或 data/trajectory 中最新记录")
        ...
        # D435i 同步采集设置
        camera_group = QGroupBox("D435i同步视频采集")
        camera_layout = QGridLayout(camera_group)
        self.check_d435i_recording = QCheckBox("记录模式下同步录制D435i视频")
        self.check_d435i_recording.setChecked(True)
        camera_layout.addWidget(self.check_d435i_recording, 0, 0)
        camera_layout.addWidget(QLabel("保存：data/video/teach_record_时间_color.mp4、depth_vis.mp4、camera_timestamps.csv、camera_meta.json"), 0, 1)
    ...
    def browse_record_file(self) -> None:
        if core is not None:
            os.makedirs(core.TRAJECTORY_DIR, exist_ok=True)
            start_dir = core.TRAJECTORY_DIR
        else:
            start_dir = os.getcwd()
        path, _ = QFileDialog.getSaveFileName(self, "选择记录文件", start_dir, "CSV Files (*.csv)")
        if path:
    ...
    def start_or_switch(self, cmd: str) -> None:
        if core is None:
            QMessageBox.critical(self, "错误", f"无法导入 teleoperation_data_caiji.py：\n{CORE_IMPORT_ERROR}")
            return
